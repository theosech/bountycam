...
export default function SessionClient({ initialSession, sessionId }: SessionClientProps) {
  ...
  const handleFinishSession = async (approved: boolean) => {
    ...
    try {
      ...
      if (session?.stream_started_at && !(session as any).stream_ended_at) {
        await supabase
          .from('sessions')
            ...
            {session.status === 'completed' && (
              <div className="bg-green-50 p-4 rounded">
                <p className="text-green-900">
                  ...
                  {(session as any).approved && ' The bounty was approved and points were transferred.'}
                </p>
              </div>
            )}
          </div>
